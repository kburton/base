...
class TbActiveForm extends CActiveForm
{
	...
	public function createStaticGroup($label, $html, $htmlOptions = array())
	{
		return $this->createCustomGroup($label,
				'<p class="form-control-static">' . $html . '</p>', $htmlOptions);
	}
	
	public function createCustomGroup($label, $html, $htmlOptions = array())
	{
		if (!isset($htmlOptions['formLayout']))
			$htmlOptions['formLayout'] = $this->layout;
		
		$htmlOptions['label'] = $label;
		
		$htmlOptions['labelWidthClass'] = $this->labelWidthClass;
		$htmlOptions['controlWidthClass'] = $this->controlWidthClass;
		
		return TbHtml::customControlGroup($html, '', $htmlOptions);
	}
